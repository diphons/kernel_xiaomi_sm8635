...
#define PROBE_FAILED_REGISTER_STUB_DAI
...
static const char * const tlv_reset_pctl_names[] = {
	"tlv_reset_io_high",
	"tlv_reset_io_low",
};
...
struct aic3x_priv {
	...
	u8 ocmv;
	struct clk *hifi_tlv_clk;
	bool is_tlv_mclk_on;
	/* aix3x reset io controller */
	bool reset_gpio_output_status;
	int enable_reset_pin_control;
	struct pinctrl *aic3x_reset_pinctrl;
	struct pinctrl_state *pinctrl_state[2];
	/* amp mute io controller */
	bool amp_mute_gpio_output_status;
};
...
/**
 * snd_soc_component_test_bits_aic3x is same as
 * snd_soc_component_test_bits
 */
static int snd_soc_component_test_bits_aic3x(struct snd_soc_component *component,
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	change = snd_soc_component_test_bits_aic3x(component, reg, mask, val);
	if (change) {
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_info(component->dev, "%s: update Mic Bias event: %d\n",
			__func__, event);
 ...
 * MCLK:
 * get MCLK status
 * powerup: enable hifi_dac_clk
 * powerdown: disable hifi_dac_clk
 */
static int lpass_tlv_clk_enable_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct aic3x_priv *aic3x;
	...
	if (!component) {
		dev_err(component->dev, "%s(): failed get snd_soc_component according to kcontrol\n", __func__);
		return -EINVAL;
	}
	aic3x = snd_soc_component_get_drvdata(component);
	if (!aic3x) {
		dev_err(component->dev, "%s(): failed get aic3x_priv according to component\n", __func__);
		return -EINVAL;
	}
...
static int lpass_tlv_clk_enable_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	struct aic3x_priv *aic3x;
	...
	if (!aic3x->hifi_tlv_clk) {
		dev_err(component->dev, "%s: Undefined HIFI TLV320 MCLK.\n",
				__func__);
		return -EINVAL;
	}

	aic3x->is_tlv_mclk_on =
			(!ucontrol->value.bytes.data[0] ? false : true);

	dev_err(component->dev, "%s: is_tlv_mclk_on=%d\n",
				__func__, aic3x->is_tlv_mclk_on);

	if (aic3x->is_tlv_mclk_on) {
		ret = clk_prepare_enable(aic3x->hifi_tlv_clk);
		if (ret < 0) {
			dev_err(component->dev, "%s: hifi_tlv_clk enable failed\n",
						__func__);
			return ret;
		}
	} else {
		clk_disable_unprepare(aic3x->hifi_tlv_clk);
	}

	return ret;
}

static int aic3x_reset_pin_ctrl_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct aic3x_priv *aic3x;
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
...
static int aic3x_reset_pin_ctrl_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	struct aic3x_priv *aic3x;
	...
	aic3x->reset_gpio_output_status = ucontrol->value.integer.value[0];
	if (aic3x->aic3x_reset_pinctrl && aic3x->enable_reset_pin_control) {
		if (aic3x->reset_gpio_output_status) {
			dev_info(component->dev, "%s: reset_gpio_output_status %d, rcv switch gpio pull up\n", __func__,
					aic3x->reset_gpio_output_status);
			pinctrl_select_state(aic3x->aic3x_reset_pinctrl,
					aic3x->pinctrl_state[0]);
		} else {
			dev_info(component->dev, "%s: reset_gpio_output_status %d, rcv switch gpio pull down\n", __func__,
					aic3x->reset_gpio_output_status);
			pinctrl_select_state(aic3x->aic3x_reset_pinctrl,
					aic3x->pinctrl_state[1]);
		}
	}
...
static int amp_mute_pin_ctrl_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct aic3x_priv *aic3x;
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
...
static int amp_mute_pin_ctrl_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	struct aic3x_priv *aic3x;
	...
	aic3x->amp_mute_gpio_output_status = ucontrol->value.integer.value[0];

	if (aic3x->amp_mute_gpio_output_status) {
		ret = tas5411_pinctrl_controller(true);
	} else {
		ret = tas5411_pinctrl_controller(false);
	}
...
static const struct snd_kcontrol_new aic3x_snd_controls[] = {
	...
	*/
	...
	/* For MCLK MODE */
	SOC_SINGLE_EXT("TLV MCLK Switch", SND_SOC_NOPM, 0, 1, 0,
			lpass_tlv_clk_enable_get, lpass_tlv_clk_enable_put),
	/* For Codec Reset IO controller */
	SOC_SINGLE_EXT("TLV Reset Pin Control", SND_SOC_NOPM, 0, 1, 0,
			aic3x_reset_pin_ctrl_get, aic3x_reset_pin_ctrl_put),
	/* For AMP Mute IO controller */
	SOC_SINGLE_EXT("AMP Mute Pin Control", SND_SOC_NOPM, 0, 1, 0,
			amp_mute_pin_ctrl_get, amp_mute_pin_ctrl_put),
};
			   ...
			   struct snd_soc_dai *dai)
{
	...
	unsigned int rate_add = params_rate(params);
	u8 asp_wl_add = params_physical_width(params);
	...
	if (aic3x->sysclk == 0) {
		dev_dbg(component->dev, "%s(): aic3x->sysclk %d, set dai sysclk\n", __func__, aic3x->sysclk);
		aic3x_set_dai_sysclk(dai, CLKIN_MCLK, 2 * rate_add * width, 0);
	}

	/* Fsref can be 44100 or 48000 */
	fsref = (params_rate(params) % 11025 == 0) ? 44100 : 48000;

	dev_dbg(component->dev, "%s():fsref %d params_rate(params) %d aic3x->sysclk %d width %d asp_wl_add %d rate_add %d\n", 
		__func__, fsref, params_rate(params), aic3x->sysclk, width, asp_wl_add, rate_add);
...
found:
	dev_dbg(component->dev, "%s(): bypass_pll %d codec_clk %d last_clk %d [P,R,J,D] = [%d,%d,%d,%d]\n",
		__func__, bypass_pll, codec_clk, last_clk, pll_p, pll_r, pll_j, pll_d);
...
#ifdef PROBE_FAILED_REGISTER_STUB_DAI
static struct snd_soc_dai_driver aic3x_stub_dai = {
	.name = "tlv320aic3x-hifi",
	.playback = {
		.stream_name = "Playback",
		.channels_min = 2,
...
static struct snd_soc_component_driver aic3x_stub_codec;
#endif
...
int tlv_reset_gpio_switch_init(struct device *dev, struct aic3x_priv *aic3x)
{
	int ret = 0, i =0;

	aic3x->enable_reset_pin_control = 0;
	aic3x->aic3x_reset_pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR(aic3x->aic3x_reset_pinctrl)) {
		if (PTR_ERR(aic3x->aic3x_reset_pinctrl) == -EPROBE_DEFER) {
			dev_err(dev, "pinctrl not ready\n");
			ret = -EPROBE_DEFER;
			return ret;
		}
		dev_err(dev, "Target does not use pinctrl\n");
		aic3x->aic3x_reset_pinctrl = NULL;
		ret = -EINVAL;
		return ret;
	}
	for (i = 0; i < ARRAY_SIZE(tlv_reset_pctl_names); i++) {
		const char *n = tlv_reset_pctl_names[i];
		struct pinctrl_state *state =
			pinctrl_lookup_state(aic3x->aic3x_reset_pinctrl, n);
		if (IS_ERR(state)) {
			dev_err(dev, "cannot find '%s'\n", n);
			ret = -EINVAL;
			return ret;
		}
		aic3x->pinctrl_state[i] = state;
		aic3x->enable_reset_pin_control = 1;
		dev_dbg(dev, "found reset pin control %s, enable_reset_pin_control %d\n", n, aic3x->enable_reset_pin_control);
	}
	return ret;
}
...
int aic3x_probe(struct device *dev, struct regmap *regmap, kernel_ulong_t driver_data)
{
	...
	struct device_node *np = dev->of_node;
	struct clk *hifi_tlv_clk = NULL;
	int ret, i;
	u32 value;
	static u32 probe_err_count = 0;
	int probe_max_count = 1;
	dev_info(dev, "aic3x probe E\n");
	...
	if (gpio_is_valid(aic3x->gpio_reset) &&
		...
		if (ret != 0) {
			dev_err(dev, "Failed to request tlv320aic3x reset: %d\n", ret);
			goto err;
		}
		gpio_direction_output(aic3x->gpio_reset, 0);
	}

	/* Add reset gpio controller for aic3x */
	ret = tlv_reset_gpio_switch_init(dev, aic3x);
	if (ret != 0) {
		dev_err(dev, "Failed to init reset gpio: %d\n", ret);
		goto err;
	}
	...
	hifi_tlv_clk = devm_clk_get(dev, "hifi_dac_clk");
	if (IS_ERR(hifi_tlv_clk)) {
		ret = PTR_ERR(hifi_tlv_clk);
		dev_dbg(dev, "%s: clk get %s failed %d\n",
			__func__, "hifi_dac_clk", ret);
		hifi_tlv_clk = NULL;
	}
	aic3x->hifi_tlv_clk = hifi_tlv_clk;

	aic3x->is_tlv_mclk_on = false;
...
err:
	...
	if (probe_err_count >= probe_max_count) {
		dev_warn(dev, "%s: Register stub codec\n", __func__);
		memset(&aic3x_stub_codec, 0, sizeof(struct snd_soc_component_driver));
		ret = devm_snd_soc_register_component(dev, &aic3x_stub_codec, &aic3x_stub_dai, 1);
		if (ret < 0) {
			dev_err(dev, "%s: Register stub codec failed\n", __func__);
		}
		probe_err_count = 0;
		return 0;
	}
