...
static void cs35l43_wm_adsp2_init_debugfs(struct wm_adsp *dsp,
...
#else
static inline void cs35l43_wm_adsp2_init_debugfs(struct wm_adsp *dsp,
					 struct snd_soc_component *component)
{
}
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_fw_get);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_fw_put);

const struct soc_enum cs35l43_wm_adsp_fw_enum[] = {
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_fw_enum);
...
int cs35l43_wm_adsp_write_ctl(struct wm_adsp *dsp, const char *name, int type,
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_write_ctl);

int cs35l43_wm_adsp_read_ctl(struct wm_adsp *dsp, const char *name, int type,
		     unsigned int alg, void *buf, size_t len)
{
	struct wm_coeff_ctl *ctl;
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_read_ctl);
...
int cs35l43_wm_adsp_load_coeff(struct wm_adsp *dsp)
{
	LIST_HEAD(buf_list);
	struct regmap *regmap = dsp->regmap;
	struct wmfw_coeff_hdr *hdr;
	struct wmfw_coeff_item *blk;
#if defined(CONFIG_TARGET_PRODUCT_AURORA)
	uint32_t hw_id_value = 0;
	uint32_t hw_version_platform = 0;
	...
	struct wm_adsp_buf *buf;
#if defined(CONFIG_TARGET_PRODUCT_AURORA)
	hw_id_value = get_hw_id_value();
	hw_version_platform = get_hw_version_platform();
        hw_country_ver = get_hw_country_version();
	adsp_info(dsp, "after hw_id_value: 0x%x hw_version_platformï¼š%d hw_country_verï¼š0x%x\n", hw_id_value, hw_version_platform, hw_country_ver);
#endif
	file = kzalloc(PAGE_SIZE, GFP_KERNEL);
	if (file == NULL)
		return -ENOMEM;
#if defined(CONFIG_TARGET_PRODUCT_AURORA)
	adsp_info(dsp, "aurora device pa bin\n");
	if( ((uint32_t)CountryCN == hw_country_ver) && !((hw_id_value == 0x1) || (hw_id_value == 0x10000) || (hw_id_value == 0x10001))){
		if (dsp->tuning_has_prefix || dsp->component->name_prefix)
			snprintf(file, PAGE_SIZE, "%s-%s-%s-%s-%s.bin",
				dsp->component->name_prefix, dsp->part,
				dsp->fwf_name, wm_adsp_fw[dsp->fw].file, "tt");
		else
			snprintf(file, PAGE_SIZE, "%s-%s-%s-%s.bin", dsp->part,
				dsp->fwf_name, wm_adsp_fw[dsp->fw].file, "tt");
	}else{
		if (dsp->tuning_has_prefix || dsp->component->name_prefix)
...
#else
	adsp_info(dsp, "other devices pa bin\n");
	if (dsp->tuning_has_prefix || dsp->component->name_prefix)
		snprintf(file, PAGE_SIZE, "%s-%s-%s-%s.bin",
			 dsp->component->name_prefix, dsp->part,
			 dsp->fwf_name, wm_adsp_fw[dsp->fw].file);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_load_coeff);
...
int cs35l43_wm_adsp1_init(struct wm_adsp *dsp)
{
	dsp->ops = &wm_adsp1_ops;

	return wm_adsp_common_init(dsp);
}
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp1_init);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp1_event);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp2_set_dspclk);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp2_preloader_get);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp2_preloader_put);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_early_event);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_event);
...
int cs35l43_wm_adsp2_component_probe(struct wm_adsp *dsp, struct snd_soc_component *component)
{
	char preload[32];
	...
	cs35l43_wm_adsp2_init_debugfs(dsp, component);

	dsp->component = component;
	
	return 0;
}
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp2_component_probe);

int cs35l43_wm_adsp2_component_remove(struct wm_adsp *dsp, struct snd_soc_component *component)
{
	wm_adsp2_cleanup_debugfs(dsp);

	return 0;
}
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp2_component_remove);

int cs35l43_wm_adsp2_init(struct wm_adsp *dsp)
{
	int ret;
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp2_init);

int cs35l43_wm_halo_init(struct wm_adsp *dsp)
{
	int ret;
...
EXPORT_SYMBOL_GPL(cs35l43_wm_halo_init);

void cs35l43_wm_adsp2_remove(struct wm_adsp *dsp)
{
	struct wm_coeff_ctl *ctl;
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp2_remove);
...
int cs35l43_wm_adsp_compr_open(struct wm_adsp *dsp, struct snd_compr_stream *stream)
{
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_compr_open);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_compr_free);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_compr_set_params);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_compr_get_caps);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_compr_trigger);
...
int cs35l43_wm_adsp_compr_handle_irq(struct wm_adsp *dsp)
{
	struct wm_adsp_compr_buf *buf;
	struct wm_adsp_compr *compr;
	int ret = 0;
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_compr_handle_irq);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_compr_pointer);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_compr_copy);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp2_bus_error);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_halo_bus_error);
...
EXPORT_SYMBOL_GPL(cs35l43_wm_halo_wdt_expire);
